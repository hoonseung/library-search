...
public class BookQueryService {
    ...
    public PageResult<SearchResponse> search(String query, int page, int size) {
        log.info("[BookQueryService] search query={}", query);
    ...
    private PageResult<SearchResponse> handleException(String query, int page, int size, Throwable throwable) {
        log.error("[BookQueryService] CallNotPermittedException occurred!! Fallback to Kakao Search errorMessage={}", throwable.getMessage());
        return kakaoBookRepository.search(query, page, size);
    }
}
